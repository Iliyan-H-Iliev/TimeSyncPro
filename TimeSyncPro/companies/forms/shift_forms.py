...
from ...accounts.form_mixins import ReadonlyFieldsFormMixin


class ShiftBaseForm(CheckExistingNamePerCompanyMixin, ReadonlyFieldsFormMixin, LabelMixin, forms.ModelForm):
    readonly_fields = [
        "rotation_weeks",
    ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['shift_members'].queryset = self.company.employees.filter(shift=None)
...
class UpdateShiftForm(ShiftBaseForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.initial_shift_members = set(self.shift.employees.all() if self.shift else [])
        self.initial_shift_teams = set(self.shift.teams.all() if self.shift else [])

        if self.company:
            sorted_shift_members = self.shift.employees.all().order_by('first_name', 'last_name')
            sorted_non_shift_members = self.company.employees.filter(shift=None).order_by('first_name', 'last_name')
            combined_queryset = sorted_shift_members | sorted_non_shift_members

            sorted_shift_teams = self.shift.teams.all().order_by('name')
            sorted_non_shift_teams = self.company.teams.filter(shift=None).order_by('name')
