    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            if 'department_members' in self.cleaned_data:
                (Profile.objects.filter(id__in=[m.id for m in self.cleaned_data['department_members']])
...
class EditDepartmentForm(DepartmentBaseForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.initial_department_members = set(self.department.employees.all() if self.department else [])

        if self.company:
            sorted_department_members = self.department.employees.all().order_by('first_name', 'last_name')
            sorted_non_department_members = self.company.employees.filter(department=None).order_by('first_name',
                                                                                                    'last_name')
            combined_queryset = sorted_department_members | sorted_non_department_members
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            members_to_remove = self.initial_department_members - final_department_members
            Profile.objects.filter(id__in=[m.id for m in members_to_remove]).update(department=None)

            members_to_add = final_department_members - self.initial_department_members
            Profile.objects.filter(id__in=[m.id for m in members_to_add]).update(department=department)
