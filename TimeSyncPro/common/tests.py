...
class IntegrationTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = UserModel.objects.create_user(email='testuser@abv.bg', password='testpassword')
    ...
    def test_create_profile_company(self):
        response = self.client.get(reverse('create_profile_company', kwargs={'slug': self.user.slug}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_absence_list(self):
        response = self.client.get(reverse('company_absences', kwargs={'company_slug': self.company.slug}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_absence_detail(self):
        ...
        response = self.client.get(reverse('employee_absences', kwargs={'slug': self.user.slug, "company_slug": self.company.slug}))
        self.assertEqual(response.status_code, 302)

    def test__create_absence_with_correct_data(self):
        response = self.client.post(reverse('add_absence', kwargs={'slug': self.user.slug, "company_slug": self.company.slug}), {
            "start_date": '2021-01-01',
            "end_date": '2021-01-03',
            'absentee': self.profile,
            'absence_type': 'sick',
    ...
    def test_delete_absence(self):
        ...
        response = self.client.post(reverse('delete_absence', kwargs={'pk': absence.pk}))
        self.assertEqual(response.status_code, 302)

    def test_profile_update(self):
        response = self.client.post(reverse('update_profile', kwargs={'slug': self.user.slug}), {
            'user': self.user.id,
    ...
    def test_company_update(self):
        response = self.client.post(reverse('update_company', kwargs={'company_slug': self.company.slug}), {
    ...
    def test_user_registration(self):
        response = self.client.post(reverse('sign_up_administrator'), {
