...
class GetQuerySetMixin:
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser or (
            ...
            and getattr(request.user.profile, "company", None) is None
        ):
            return qs
...
class TSPUserAdmin(GetQuerySetMixin, UserAdmin):
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if request.user.is_superuser:
            ...
            and request.user.profile.is_company_admin
        ):
            return (
                (None, {"fields": ("email", "password")}),
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if not (
            hasattr(request.user, "profile") and request.user.profile.is_company_admin
        ):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if not request.user.is_superuser:
            ...
            field_querysets = {
                ...
                "department": company_with_related_data.departments.filter(
                    company=request.user.profile.company
                ),
                "team": company_with_related_data.teams.filter(
                    company=request.user.profile.company
                ),
                "shift": company_with_related_data.shifts.filter(
                    company=request.user.profile.company
                ),
                "employee": company_with_related_data.employees.filter(
                    company=request.user.profile.company
                ),
            }
